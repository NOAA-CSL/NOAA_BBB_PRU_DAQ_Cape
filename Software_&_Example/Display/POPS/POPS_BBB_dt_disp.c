...
//  programs.
//  3.0 Made changes to allow the output to be displayed by another BBB with a 
//  4DCape-43T. 3/23/2017 L A Watts.
*/
/*DISCLAIMER
----------------------------------------------
...
printf("afteruid check")	;
//******************************
// Read the configuration file
//******************************

    Read_POPS_cfg();
printf("after read cfg");
//******************************
// Initialize the time and Files
//******************************

    getTimes();
printf("after get times");
    makeFileNames();
...
void getTimes(void)
{
    ...
    sprintf(gDispTime, "%Y-%m-%d %H:%M:%S", (gmtNow->tm_year+1900), \
    (gmtNow->tm_mon+1), (gmtNow->tm_mday),(gmtNow->tm_hour),(gmtNow->tm_min), \
    (gmtNow->tm_sec));
...
void makeFileNames(void)
{
    ...
    char binNames[1000];                    // Histogram Bin names
    char str[1000];                         // string dummy
...
else if (strcmp(gStatus_Type, "Display") ==0)
    {
        strcpy(gStatus, gDispTime);
        strcat(gStatus," ");
        strcat(gStatus, gPeakFile);

...
//  programs.
//  3.0 Add read aircraft data. This changes the UDP communications and Write Files
*/
/*DISCLAIMER
----------------------------------------------
...
//*****************************
    ...
    if(gUDP.udp[0].use) UDP0S = Open_Socket_Write(0);
...
void makeFileNames(void)
{
    ...
    char binNames[1000];                    // Histogram Bin names (max = 883 char)
...
// Status Packet
    if (strcmp(gStatus_Type, "Display") ==0)
    {
    ...
    else if(!strcmp(gStatus_Type, "iMet"))
    {
...
/* bind it to all local addresses and pick the port number */
    ...
    gUDP.udp[i].myaddr.sin_family = AF_INET;
    gUDP.udp[i].myaddr.sin_addr.s_addr = htonl(INADDR_ANY);
    gUDP.udp[i].myaddr.sin_port = htons(gUDP.udp[i].port+1);

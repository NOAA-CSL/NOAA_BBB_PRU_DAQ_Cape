...
//sprintf(str,"LoopStart.tv_sec = %d LoopStop.tv_sev = %d \n",LoopStart.tv_sec, LoopStop.tv_sec);
//printf("LoopStart.tv_sec = %d LoopStop.tv_sev = %d \n",LoopStart.tv_sec, LoopStop.tv_sec);
//strcat(gMessage,str);
...
// UART1 Status ****************************************************************
		...
		{
			//printf("Serial 1 not open, reopen\n");
			Close_Serial(UART1);
			UART1 = Open_Serial(gSerial_Ports.serial_port[0].port,
				gSerial_Ports.serial_port[0].baud);
		}
...
// UART2 Full data and Raw data ************************************************
		...
		{
			//printf("Serial 2 not open, reopen\n");
			Close_Serial(UART2);
			UART2 = Open_Serial(gSerial_Ports.serial_port[1].port,
				gSerial_Ports.serial_port[1].baud);
		}
...
// Loop on Process and Baseline until 1 sec or stop ****************************
	...
	//	printf("j last loop = %d\n",j);

		Check_Stop();
		if(gStop) goto Shutdown;
		LoopStart = LoopStop;
		LoopStop.tv_sec +=1.;	// Add one sec to the loop tamer for the next iteration
...
//************************************************
Shutdown:
printf("gStop = %d system shutdown\n",gStop);
	gAO_Data.ao[0].set_V = 0.0;
	gAO_Data.ao[1].set_V = 0.0;
	Set_AO(0,0.0);		//Set AO0
		...
		switch(gAI_Data.ai[i].conv)
		{
		   ...
		   case Therm:							//Thermistor T
		   {
			...
			int3 = D*LR*LR*LR*LR*LR;
			  ...
			  //printf(" lr %.4g int1  %.4g, int2  %.4g, int3 %.4g, T %.4g\n",LR,
			  //int1,int2,int3,gAI_Data.ai[i].value );
...
//*****************************************************************************
//
// Gather the Status information and full data to send
//
//*****************************************************************************
void POPS_Output (void)
{
...
// Status Packet
		if(!strcmp(gStatus_Type, "iMet"))
		{
			...
			temp_out = (int)(100+gAI_Data.ai[5].value);				//Temperature
...
{
   int file;
   struct termios options;		 //The termios structure is vita
...
int Send_Serial(int UART, char msg[])
{
   ...
   if ((count = write(UART, msg, (strlen(msg))))<0)		//send the string without
														//null termination
   {
	 strcat(gMessage,"Failed to write to the output - uart1\n");
	 printf("failed to write uart %d\n ",UART);
	 return -1;
   }
...
//	printf("BL %x, BLTH %x, STD %.2f, th_mult %.2f\n",gBaseline, gBLTH, gSTD, gTH_Mult);
...
//*****************************************************************************
//
// Read the raw data stored in PRU0 memory
// Use 16 bit packed data into 1 KB. (512 points).
//
//*****************************************************************************
...
//	clear the Bins
//	memset(gHist, 0, sizeof(gHist[0]*gBins.nbins));
	for(i=0; i<gBins.nbins;i++)
	{
		gHist[i]=0;
	}
	if(gArray_Size == 0) return;		// Don't try binning is there are no particles
...
MAX5802_status Set_AO(unsigned int n, double Read_V)
{
	...
	//printf("AO set ch= %u V=%.2f set=%u \n", n, V_set_in, gAO_Data.ao[n].set_num);
...
*
* \param[in]	uchReferenceCommand	 - specifies voltage reference setting desired
*
* \retval		MAX5802_status
*************************************************************************************/
MAX5802_status MAX5802_set_internal_reference( short unsigned int uchReferenceCommand)
{
...
*
* \param[in]	uchMask				 - selects which DACs to affect, 1 bit per DAC
* \param[in]	uchDefaultCodeSetting	 - determines which default state to set dac's
*											CODE register
*
* \retval		MAX5802_status
********************************************************************************/
...
* \retval		MAX5802_status
********************************************************************************/
MAX5802_status MAX5802_initialize()
{
...
*
* \param[in]	uchChannelToSet		 - dac channel# (0-7) to load (8=all)
* \param[in]	unRegisterSetting		 - value to load into the CODE register
*
* \retval		MAX5802_status
*************************************************************************************/
...
* 		
*		It is commonly used with the constant setting approximate delays.
*
...
//*********************************************************************
//
// Clock in command to the ms5607
//		8bit command, sent low bit first
...
// *
// * Description:	Send Reset command to MS5607 and wait 10ms each
// *
// * Parameters:	none
// *
// *
...
/*********************************************************************
 *
 * Function:	 ms5607_read_prom
 *
 * Description:	Read factory calibrated coefficients from PROM
 *
 * Parameters: void
 *
 * Returns:		Enumerated type ms5607_status with possible values
 *				ms5607_status_ok or ms5607_status_crc_error
 *
 *********************************************************************/
ms5607_status ms5607_read_prom(void)
{
...
/*********************************************************************
 *
 * Function:	 ms5607_read_TP
 *
 * Description:	Send commands to start a temperature conversion,
 *				wait for completion, read the temperature value, start
 *				a pressure conversion, wait for completion and read the
 *				pressure value
 *
 * Parameters: used globals
 *
 * Returns:		Enumerated type ms5607_status with possible values
 *				ms5607_status_ok or ms5607_status_i2c_transfer_error
 *				or ms5607_status_crc_error
 *
 *********************************************************************/
ms5607_status ms5607_read_TP(void)
{
...
/*********************************************************************
 *
 * Function:	 ms5607_CRC4
 *
 * Description:	Does a CRC4 check on the 7 prom coefficients
 *
 * Parameters: used globals
 *
 * Returns:		Enumerated type ms5607_status with possible values
...
//******************************************************************************
//
// Initialize the PRU memory space: used in program testing only.
//
//******************************************************************************
void InitPRU_Mem(void)
{
...
// y_all is the new y data, 16 bytes at a time
	...
	for (j=0; j< i_tail; j+=2)
	{
		...
		//4 bytes to dt in uS. 16 added to make up for lost cycles.
		gArray_Size+=1;
		}
	}
}
/*********************************************************************
 *
 * Function:	 Open_Socket_Write
 *
 * Description:	Opens a UDP socket for write
 *				
 * Parameters: UDP index
 *
 * Returns:		fd
 *				
 *********************************************************************/
...
/*********************************************************************
 *
 * Function:	 Open_Socket_Broadcast
 *
 * Description:	Opens a UDP socket for broadcast
 *				
 * Parameters: UDP index
 *
 * Returns:		fd
 ...
 * Function:	 Write_UDP
 *
 * Description:	Writes message to a UDP socket
 *				
 * Parameters: fd, udp index, message
 *
 * Returns:		count
 *				
 *********************************************************************/
...
/*********************************************************************
 *
 * Function:	 Open_Socket_Read
 *
 * Description:	Opens a UDP socket for read
 *				
 * Parameters: UDP index
 *
 * Returns:		fd
...
/*********************************************************************
 *
 * Function:	 Read_UDP_Data
 *
 * Description:	Read data from a UDP socket
 *				
 * Parameters: UDPID, BufferSize
 *
 * Returns:		int -1 = no data, >=0 data present
 *				
 *********************************************************************/
int UDP_Read_Data(int fd, int i)
{
...
//	reclen = recvfrom(fd, gCMD, 512, 0, (struct sockaddr *)&gUDP.udp[i].remaddr, &addrlen);
